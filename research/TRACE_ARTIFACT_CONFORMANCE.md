...
The conformance evidence below records the historical 18-case corpus replay and the extended 256-world experiment. The current clean reproducibility runner does not freshly execute every historical resolver incident in the corpus; it does regenerate the 256-world projection harness, one executable proof-minimality check, and the 250-case mutation campaign. See `research/REPRODUCIBILITY_INDEX.md`.

The 18/18 corpus result remains a historical serialized research record; the current clean runner does not freshly execute every historical resolver incident. The 256-world projection result is regenerated by the current projection harness.
